...
public class CityConnect {
	...
	private static String getDistance(String userCommand) {
		...
		if (hasExistingRoute(newStartLocation, newEndLocation)){
			return String.format(MESSAGE_DISTANCE, newStartLocation, newEndLocation,
					route[NOT_FOUND][STORAGE_POSITION_DISTANCE]);
		} 
		else 
		{
	...
	//Extracted checking for existing routes out of getDistance
	private static boolean hasExistingRoute(String newStartLoc, String newEndLoc){
		int position = getPositionOfExistingRoute(newStartLoc, newEndLoc);
